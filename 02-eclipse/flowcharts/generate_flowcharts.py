...
def overview_flowchart():
    ...
    ax.set_ylim(0, 15)
    ax.axis('off')
    ax.set_title('Eclipse -- Hybrid AI + Heuristic Cargo Packer -- Overview', fontsize=15, fontweight='bold', pad=10)
    ...
    steps = [
        ...
        ('Hybrid 3D packer -- Eclipse competes here:\ntrained RL placement policy + 4 heuristics,\nbest-of-N per container (+339pt measured)', COLOR_HYBRID),
...
def detailed_flowchart():
    ...
    ax.set_ylim(0, 14)
    ax.axis('off')
    ax.set_title('Eclipse -- Hybrid AI + Heuristic Cargo Packer -- Detailed Pipeline', fontsize=16, fontweight='bold', pad=10)
    ...
    box(ax, (1, 5.7), 13.8, 1.1, 'Hybrid 3D Packer -- REAL evaluation (per container, best-of-N):\n'
                                  'Eclipse (trained RL placement policy)  |  4 heuristic strategies (contact-area, deepest-corner) x2 candidate-generation methods',
        COLOR_HYBRID, fontsize=8.5)
    arrow(ax, (11.5, 7.2), (7.9, 6.8))

    arrow(ax, (12.85, 10), (4, 8.4))  # seed order feeds into loop
    ...
    box(ax, (5.5, 0.6), 5, 1.3, 'Final packing solution\n(28,452 on the real benchmark)', COLOR_OUTPUT, fontsize=10)
