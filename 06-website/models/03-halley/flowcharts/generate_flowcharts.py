...
def overview_flowchart():
    ...
    ax.set_ylim(0, 15)
    ax.axis('off')
    ax.set_title('Halley -- Hybrid AI + Heuristic Cargo Packer -- Overview', fontsize=15, fontweight='bold', pad=10)
    ...
    steps = [
        ...
        ('Halley: Economy package order\n(PackageSetRanker, GRPO-trained\nacross many instances)', COLOR_AI),
        ('Hybrid 3D packer\n(Eclipse RL placement policy +\nheuristics, best-of-N per container)', COLOR_HYBRID),
...
def detailed_flowchart():
    ...
    ax.set_ylim(0, 14)
    ax.axis('off')
    ax.set_title('Halley -- Hybrid AI + Heuristic Cargo Packer -- Detailed Pipeline', fontsize=16, fontweight='bold', pad=10)

    # Stage 1: input + priority assignment
    box(ax, (0.5, 12), 4, 1.3, 'Input CSV\n(packages, ULDs, K)', COLOR_INPUT, fontsize=9)
    box(ax, (5.2, 12), 4.3, 1.3, 'Priority Clusterer\n(trained neural network,\ndifferent model from Halley)', COLOR_AI, fontsize=8.5)
    box(ax, (10.2, 12), 5.3, 1.3, 'Priority -> ULD partition\n+ exhaustive Priority packing', COLOR_HEURISTIC, fontsize=9)
    arrow(ax, (4.5, 12.65), (5.2, 12.65))
    arrow(ax, (9.5, 12.65), (10.2, 12.65))

    # Stage 2: economy seed -- Halley's stage
    box(ax, (10.2, 10), 5.3, 1.3, 'Halley: PackageSetRanker scores\nevery Economy package, sort desc\n(set-attention, GRPO-trained)', COLOR_AI, fontsize=8.5)
    arrow(ax, (12.85, 12), (12.85, 11.3))

    # Stage 3: local search loop (big box)
    loop_x, loop_y, loop_w, loop_h = 0.5, 5.3, 15, 4.2
    ...
    box(ax, (1, 7.2), 6, 1.2, 'Candidate moves\n(swap / relocate / block-shuffle)', COLOR_HEURISTIC, fontsize=8.5)
    box(ax, (8.3, 7.2), 6.5, 1.2, 'All candidates real-evaluated directly\n(no pre-screening in the winning run)', COLOR_HEURISTIC, fontsize=8.5)
    arrow(ax, (7, 7.8), (8.3, 7.8))

    box(ax, (1, 5.7), 13.8, 1.1, 'Hybrid 3D Packer -- REAL evaluation (per container, best-of-N):\n'
                                  'Eclipse (RL placement policy)  |  4 heuristic strategies (contact-area, deepest-corner) x2 candidate-generation methods',
        COLOR_HYBRID, fontsize=8.5)
    arrow(ax, (11.5, 7.2), (7.9, 6.8))

    arrow(ax, (12.85, 10), (4, 8.4))  # Halley's order feeds into loop
    arrow(ax, (7.9, 5.7), (7.9, 4.5))  # loop feeds down toward decision
    ...
    # Stage 4: decision
    box(ax, (5.5, 2.8), 5, 1.3, 'Keep best-cost candidates\n(elitist selection, deduplicated)', COLOR_HEURISTIC, fontsize=9)
    arrow(ax, (7.9, 2.8), (7.9, 2.1))
    arrow(ax, (10.5, 3.45), (11.5, 5.3), color='#999999')
    ax.text(11.7, 4.3, 'repeat until\nplateau', fontsize=8.5, style='italic', color='#555555')

    box(ax, (5.5, 0.6), 5, 1.3, 'Final packing solution\n(30,608 standalone on real benchmark)', COLOR_OUTPUT, fontsize=9.5)
