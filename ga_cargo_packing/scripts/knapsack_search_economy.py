...
def main():
    ...
    def refill(econ_assignment, remaining_vol, remaining_wt, priority_uld=None):
        """priority_uld: if given, try filling THIS container first with the
        best-available unassigned packages before the general cross-container
        pass. Without this, a move that deliberately frees one container
        (multi_evict) gets undone by refill's normal behavior -- it fills
        whichever container comes first in a fixed iteration order, which
        usually isn't the one just freed, so the intended "room for a
        better combination here" effect mostly doesn't happen."""
        ...
        uld_order = ([priority_uld] + [u for u in uld_ids if u != priority_uld]) if priority_uld else uld_ids
    ...
    def mutate(econ_assignment, remaining_vol, remaining_wt, mode):
        ...
        if mode == 'swap' and assigned and unassigned:
            ...
            if rv[u] + vol(a) + 1e-6 >= need_v and rw[u] + wt(a) + 1e-6 >= need_w:
                ...
                rv[u] += vol(a) - need_v
                rw[u] += wt(a) - need_w
        elif mode == 'multi_evict' and assigned:
            # "Few-for-few" move: evict SEVERAL packages from one container
            # at once (not just one), then let the greedy refill() below --
            # the same logic every other move already relies on -- decide
            # what's most valuable to put back, across the whole freed
            # capacity at once rather than one slot at a time. A bigger,
            # more structural move than swap/two_for_one, same idea as
            # block_shuffle was for the order-based search: single-package
            # moves exhaust a local optimum; a coarser move can still find
            # room they can't.
            by_uld = {}
            for pid in assigned:
                by_uld.setdefault(child[pid], []).append(pid)
            candidate_ulds = [uid for uid, pids in by_uld.items() if len(pids) >= 2]
            if candidate_ulds:
                u = rng.choice(candidate_ulds)
                in_u = by_uld[u]
                k = rng.randint(2, min(4, len(in_u)))
                evicted = rng.sample(in_u, k)
                for pid in evicted:
                    child[pid] = 'NONE'
                    rv[u] += vol(pid)
                    rw[u] += wt(pid)
        ...
        refill(child, rv, rw, priority_uld=priority_uld)
        return child
    ...
    if os.path.exists(STATE_PATH):
        ...
        start_round = 0
        print(f'Initial real cost: {cost0:,.0f}  (competitor target: 29,203)')
    ...
    for round_idx in range(start_round, start_round + args.rounds):
        ...
        for cost, econ_assignment in beam:
            ...
            for _ in range(args.children_per_parent):
                mode = rng.choice(['swap', 'swap', 'relocate', 'two_for_one', 'multi_evict', 'multi_evict'])
