...
class TransformerClusterer(nn.Module):
    def __init__(self, d_model=D_MODEL, n_heads=N_HEADS, n_layers=N_LAYERS,
        ...
        # NOTE: K is deliberately NOT injected into the IL model. K only
        # matters for the RL reward (cost = K*n_priority_ulds + delay_cost);
        # the GA labels this model imitates are generated without any
        # reference to K, so a k_proj layer here would only ever add noise to
        # this stage's gradients, never useful signal (confirmed empirically:
        # a from-scratch IL retrain with k_proj added converged to a
        # meaningfully worse checkpoint than this K-blind architecture, both
        # in fewer epochs and at a higher final val_loss). K-awareness is
        # added only in src/rl/model.py, at the RL fine-tuning stage.
    ...
    def sample_actions(self, uld_feats, pkg_feats, key_padding_mask,
        ...
        for i in range(n_pkgs):
            ...
            # Priority fallback — if all ULDs blocked, force least-loaded
            if is_priority and all(lg[j].item() < -1e8 for j in range(n_ulds)):
                fallback     = min(range(n_ulds), key=lambda j: weight_used[j])
                lg[fallback] = 0.0
            ...
            if is_priority and action == MAX_N_ULDS:
                action = min(range(n_ulds), key=lambda j: weight_used[j])
