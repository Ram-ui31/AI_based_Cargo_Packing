...
from .config import (
    ...
    MAX_DELAY_COST, MAX_TIGHTNESS, K_LOG_MIN, K_LOG_MAX,
    MAX_SAFE_PKGS, MAX_SAFE_ULDS,
    DEVICE,
)


def normalize_k(k_value: float) -> float:
    """Log-scale normalization to [0, 1] -- see config.py's K_LOG_MIN/MAX docstring."""
    k_log = math.log10(max(float(k_value), 1.0))
    return min(max((k_log - K_LOG_MIN) / (K_LOG_MAX - K_LOG_MIN), 0.0), 1.0)
...
def build_tensors(packages_df, ulds_df, device, k_value):
    """
    Build model input tensors for one instance. Tightness and k_feat are
    separate (1,1) tensors.

    k_value : required, not defaulted -- K (the spread-cost coefficient) was
        previously invisible to the model, only used externally in the RL
        reward. Passing it here (log-normalized, see normalize_k()) and
        encoding it via model.py's dual k_proj/head-concat path lets the
        model actually condition its assignment strategy on K instead of
        producing the same spread regardless of whether K=100 or K=5000.
        Required rather than defaulted so a forgotten call site fails
        loudly instead of silently training/running with K=0.
    """
    ...
    return {
        ...
        'k_feat':           torch.tensor([[k_norm]], dtype=torch.float32).to(device),
    }
...
def il_sample_assignment(il_model, tensors, n_ulds, n_pkgs,
    ...
    # Sampling here is pure inference (il_model is never backpropped through
    # in this function -- callers only consume the returned assignments and
    # a detached/cached copy of il_logits for KL/baseline purposes), so the
    # whole per-package decision loop below can run on a single CPU copy of
    # the logits instead of paying a GPU sync on every .item() call, same
    # fix as model.sample_actions() and rl_assign_argmax_safe(). n_samples
    # independent draws multiply this loop's cost, so it matters here too.
    il_logits_cpu = il_logits.detach().cpu()
...
def il_sample_assignment_safe(il_model, packages_df, ulds_df, device, k_value,
                               n_samples=8, temperature=1.0,
    ...
    if not needs_chunking(packages_df, ulds_df, max_pkgs, max_ulds):
        tensors = build_tensors(packages_df, ulds_df, device, k_value)
...
def actions_to_assignment(actions, n_pkgs, packages_df, ulds_df):
    ...
    pkg_ids = packages_df['Package_ID'].tolist()
    uld_ids = ulds_df['ULD_ID'].tolist()
    # Single batched .tolist() (one sync) instead of two .item() calls per
    # package (up to MAX_N_PKGS calls, each its own MPS sync).
    actions_list = actions[:n_pkgs].cpu().tolist()
    return {pkg_ids[i]: ('NONE' if actions_list[i] == MAX_N_ULDS
                         else uld_ids[actions_list[i]])
